...
class CameraInfoYamlizerNode:
    ...
    def get_imu_tf(self):
        ...
        imu_tf = tf_buffer.lookup_transform(self.imu_frame_id,
                                             "jackal1/base_link",
                                             rospy.Time.now(),
                                             rospy.Duration(5.0))

        extrinsics_trans = [imu_tf.transform.translation.x,
                                                    imu_tf.transform.translation.y,
                                                    imu_tf.transform.translation.z]
        extrinsics_quat = [imu_tf.transform.rotation.x,
                                                   imu_tf.transform.rotation.y,
                                                   imu_tf.transform.rotation.z,
                                                   imu_tf.transform.rotation.w]
        print("Recieved imu transform message")
        body_pose_imu = transformations.quaternion_matrix(extrinsics_quat)
        body_pose_imu[:,3] = np.array([extrinsics_trans + [1]])
        body_pose_imu = np.reshape(body_pose_imu, -1).tolist()
        print(f"translation: {extrinsics_trans} quat: {extrinsics_quat}")
